...
all: apt ripgrep fzf tmux go nvim ohmyzsh nvm python3.13 formatters bat docker lsps ansible linters
...
.pyenv/shims/python3.13: .pyenv/bin/pyenv
	@figlet python3.13
	pyenv install 3.13
	pyenv global 3.13

python3.13: .pyenv/shims/python3.13
...
apt:
	...
	sudo apt update
	sudo apt install -y curl zip unzip shellcheck figlet libfuse2 zsh direnv sqlite3 plantuml polybar xclip
	sudo apt autoremove -y
...
/usr/local/bin/tmux:
	...
	cd ~/tooling/tmux; git checkout 3.5a; sh autogen.sh; ./configure; make && sudo make install
...
.local/bin/lua-language-server:
	...
	ln -s $(HOME)/.local/share/nvim/luals/bin/lua-language-server .local/bin/lua-language-server
